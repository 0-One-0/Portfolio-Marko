...
export default function FloppyDisk({ title, bgColor, acColor }) {
  const containerRef = useRef(null)
   ...
   useGSAP(() => {
    ...
    gsap.set(containerRef.current, {scale: 0.6})
  ...
  return (
    <>
      <div ref={containerRef} className={` flex ${bgColor} flex-col w-26 h-29 p-2 rounded-tr-xl rounded-sm gap-3 items-center pt-0 pb-0 box-border`}>
        <div className={` ${acColor} h-8 w-20 p-1 flex justify-end px-1 rounded-b-sm`}>
